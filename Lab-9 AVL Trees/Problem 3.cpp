...
public:
	...
	void updateHeight(Node* n)//This function updates the height of every node.
	{
		if (n == NULL)
			return;
	...
	void rotationar(Node* root)//Rotate the tree accordingly
	{
		int BFROOT, BFRLEFT, BFRRIGHT;
		BFROOT = Balance_Factor(Root);
		BFRLEFT = Balance_Factor(Root->Left);
	...
	void BST2AVL(Node* BSTRoot)//Convert BST to AVL.
	{
		if (BSTRoot == NULL)
			return;
		BST2AVL(BSTRoot->Left);
		BST2AVL(BSTRoot->Right);
		rotationar(BSTRoot);
	}
};
